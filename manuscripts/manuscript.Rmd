...
abstract: |
          Model uncertainty and limited data are fundamental
          ...
          Stochastic Dynamic Programming (SDP) framework in order to make
          robust management predictions with both model uncertainty and
          limited data. We use simulations to evaluate this
          ...
          that guarantee extinction. The GPDP 
          ...
          to population crashes, while management based on the GPDP 
          does not, since it does not underestimate the uncertainty
...
We let $X_t$ denote the size (numbers or biomass) of the focal population at time $t$ and assume that in the absence of take its dynamics are. NOTE: EQUATION NUMBERS ARE ALL MESSED UP. THIS SHOULD BE EQ 1, BUT DOES NOT APPEAR ON THE PDF YOU SENT TO ME
...
In order to find that policy, we introduce the value function$V_t(x_t)$ representing the total discounted catch from time $t$ onwards given that $X_t=x$.  This value function satisfies an equation of SDP  [ @Mangel1988; @Clark2000; @Clark2009], sometimes called the Bellman-Hamilton-Jacobi equation [@Mangel2014],

\begin{equation}
V_t(X_t) = \max_{h_t} \lbrace R(h_t, X_t) + \delta \cdot \mathbf{\mathrm{E}}_{X_t+1} \left[ V_{t+1}( X_{t+1}) | X_t, h_t \right] \rbrace
\end{equation}

where expectation is taken over all possible values of the next state, $X_t{+1}$, and maximized over all possible choices of harvest, $h_t$.  That is, at time $t$, when population size is $x$ and harvest $h_t$ is applied, the immediate return is $R(h_t, X_t)$. In the classical case, the sole source of uncertainty is the process stochasticity term, $Z$, and thus the expectation above is equivalent to taking expectations over $Z$. That is
...
We use vague inverse Gamma priors on both the lengthscale $\ell$ and $\sigma$, of the form
...
(see parameters in table S5). Since the posterior distributions differ
...
consider 96 different scenarios, described in detail below.   To begin, to help  better
...
NOTE: IN THE PDF YOU SENT THERE IS WEIRD STUFF ON LINE 237
...
NOTE: THERE IS NO MENTION OF FIGURE 3 IN THE TEXT
...
In Figure 5, we show the consequences of managing 100 replicate realizations of the simulated
fishery under each of the policies estimated..
As anticipated from the policy curves, the structurally correct model
under-harvests, leaving the stock to vary around its unfished optimum.
...
both a Latin hypercube approach and a more systematic investigation of the
...
unable to harvest the population effectively (e.g. noise large enoug to violate the
...
NOTE: THE PDF HAS TWO }} IN LINE 351
...
setA = c(r = p1[1], K = p2[1], theta = p3[1])
setB = c(r = p1[2], K = p2[2], theta = p3[2])
setC = c(r = p1[3], K = p2[3], C = p3[3])
setD = c(r = p1[4], K = p2[4], C = p3[4])
