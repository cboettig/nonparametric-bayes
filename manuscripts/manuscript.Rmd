...
source("components/sensitivity-trends.Rmd")
```
...
That is, at time $t$, when population size is $x$ and harvest $h_t$ is applied, the immediate return is $R(h_t, x_t)$ and the population size after the take is $x-h_t$, which is then translated into $X_{t+1}$ by Eqn 1. We consider the finite time problem with $T=$`r MaxT`, which we solve using value iteration algorithm [@Mangel1988; @Clark2000].
...
A GP is an infinite dimensional (because it considers a continuous random
...
$\varepsilon$ IID normal, variance $\sigma_n^2$. Then under the GP, the predicted data $X_p$ given the observations obeys
