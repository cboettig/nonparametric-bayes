...
To construct the value function, we consider a return when $X_t=x$ and harvest $h_t=h$ denoted by
$\Pi(x,h)$. For example, if the return is simply the harvest at time $t$, then $\Pi(x,h)=min(x,h)$. We assume that future harvests are discounted relative to current ones at a constant rate of discount $\delta$ and ask for the harvest policy that maximizes total discounted harvest between the current time $t$ and a final time $T$, that is we seek to maximize over choices of harvest $\mathbf{E_Z}[ \sum_{t = 0}^{T}  \Pi_t(X_t, h_t) \delta^t]$, where the state dynamics are given by Eqn 1 and $\mathbf{E_Z}$ denotes the expectation over the process stochasticity.
...
\begin{equation}
V_t(x) = \max_{h_t} \left[\Pi(h_t, x_t) + \delta \cdot \mathbf{E_Z}  V_{t+1}( Z_{t+1} f(x - h_t)) \right]
\end{equation}


That is, at time $t$, when population size is $x$ and harvest $h_t$ is applied, the immediate return is $\Pi(h_t, x_t)$ and the population size after the take is $x-h_t$, which is then translated into $X_{t+1}$ by Eqn 1. We consider the finite time problem with $T=$`r Tmax`, which we solve using value iteration algorithm [@Mangel1988; @Clark2000].
...
The nonparametric moniker for the
...
<!-- FIXME Marc suggests putting the full GP formulation in here?? -->
