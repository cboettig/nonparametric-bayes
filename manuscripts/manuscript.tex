...
never know the ``true'' model. In addition, by choosing one-dimensional
...
parametric models. Finally, the parametric models we consider are
commonly used in modeling stock-recruitment dynamics or to model sudden
...
say $x$ and $y$, generates a covariance matrix whose $i,j$th element is
given by
...
steady state scales as $\exp\left(\frac{L^2}{\sigma^2}\right)$
