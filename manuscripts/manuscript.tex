...
applications, since (a) exploring the space of all possible actions
...
hence estimates of the uncertainty can be as important as estimates of
the average or expected outcome.

Concerns about tipping points in ecological dynamics (Scheffer et al.
2001, Polasky et al. 2011) highlight this problem because they arise
from this very situation. In such cases, precise models are not
available and data is limited or biased to a particular region of state
space, such as high stock levels or an otherwise desirable state for the
ecosystem. If such models or data were available, one would know just
how far a system could be pushed before going over the tipping point,
and management would be simple. The fact that this concern is frequently
discussed highlights how often we have only limited data and inperfect
models. Moreover, the case of tipping points highlights how even small
errors can have very large consequences, as we shall illustrate later.
...
substantial structural uncertainty. We use \emph{robust} to distinguish
approaches that provide nearly optimal solutions (as measured by the
chosen value function) without being sensitive to the choice of the
(unknown) underlying model.
...
dynamics are. (Throughout, we will use $X(t+1)$ with a capital $X$ as a
reminder that this is a stochastic, uncertainty quantity at time $t$,
...
describe three choices for $f(X(t),p)$ in the next section.In this
...
To construct the value function, we consider a return when $X(t) = x $
and harvest $h(t) = h$ denoted as the reward, $R(x, h)$. For example, if
the return is simply the harvest at time $t$, then
$R(x, h) = \min(x, h)$. We assume that future harvests are discounted
relative to current ones at a constant rate of discount $\delta$ and ask
for the harvest policy that maximizes total discounted harvest between
the current time $t$ and a final time $T$. That is, we seek to maximize
over choices of harvest
$\mathbf{E}_{X(t+1)} [ \sum_{t = 0}^{T}  R(X(t), h(t), t) \delta^t]$,
...
2009), sometimes called the Hamilton-Jacobi-Bellman equation (Mangel
2014),
...
is applied, the immediate return is $R(h(t), x(t))$. In the classical
case, the sole source of uncertainty is the process stochasticity term,
$Z$, and thus the expectation above is equivalent to taking expectations
over $Z(t)$. That is
...
translated into $X(t+1)$ by Eqn 1 (that is, we just replaced $X(t+1)$
with $Z(t) f(x(t)-h(t))$).
...
which we parameterize as follows for $f(S(t),p)$ in Eqn 1. In all three,
...
We assume that the data $X(t)$ are observed with some process
stochasticity,
...
We consider the fase where we have some previous observations of the
\emph{timeseries}, $X_{\textrm{obs}}(t)$, and wish to \emph{predict}
future values $X_{\textrm{p}}(t)$. We consider the observations as pairs
of points $X_{\textrm{obs}}(t), X_{\textrm{obs}}(t+1)$ for
$t \in [1,2, ..., T]$ up through observed time $T$. Under the GP, a
finite sample of these predictions at $n$ points into the future,
$X_{\textrm p}(\tau)$ for $\tau \in [T+1, T+2, ..., T+n]$, is simply
given by a multivariate normal ($\mathcal{N}$):
...
Where $K$ is the chosen covariance kernel. (Compare to Eqn 2.22 of
Rasmussen and Williams (2006) for a more detailed derivation.) Many
options are possible for this kernel. Throughout, we use the radial
basis function for simplicity, a matrix whose $(i,j)$th element is given
by:
...
scale of the process noise by which observations $Y(t+1)$ may differ
from their predicted values $X(t+1)$ given an observation of the
previous state, $x(t)$ (see Munch et al. 2005a).
...
$X(t+1)$ given any current state $x(t)$ and any harvest $h(t)$ (See the
...
Bayesian Information Criterion) all (wrongly) select the simplest model.
As the true (Allen) model is not distinguishable from the simpler
(Ricker) model in the region of the observed data, this error cannot be
avoided regardless of the model choice criterion. This highlights the
danger of model choice when the selected model will be used outside of
...
tipping point dynamics merely on account of them being similar. The data
shown comes only from the basin of attraction near the unfished
equilibrium, near which all of the models are approximately linear and
approximately identical; so it is intuitive that all criteria choose the
simplest model. In a complex world, the result is suboptimal. But in a
world that might contain tipping points, the result is disastrous.
...
combined with our obvious inability to formulate accurate population
models for low population sizes in the absence of any data, makes this
situation the rule more than the exception. Relying on parametric models
and model choice processes that favor simplicity ignores this basic
fact. Acknowledging this, Carl Walters (e.g. Walters and Hilborn 1978)
has long observed that if we began by fishing any newly exploited
population down to very low levels and then let it recover, we would be
much better at estimating population dynamics and thus predicting the
optimal harvest levels. While certainly true, this presents a rather
risky policy in the face of potential tipping points. The GPDP offers an
alternative way to acknowledge this uncertainty.
...
approach of the GPDP, in contrast, allows a predictive model that
expresses a great deal of uncertainty about the probable dynamics
\emph{outside} the observed range, while retaining very good predictive
accuracy \emph{inside} the range observed. The management policy
dictated by the GPDP balances this uncertainty against the immediate
value of the harvest, and acts to stabilize the population dynamics in a
region of state space in which the predictions can be reliably reflected
by the data.
