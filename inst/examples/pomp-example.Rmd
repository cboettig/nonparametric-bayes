...
```{r}
theta.guess <- theta + abs(rnorm(length(theta)))
estpars <- names(theta) # which pars should be estimated. lets try and get them all
rw.sd <- theta/(50*theta) # a named vector of random walk values. This sets it to 0.02 for all pars
system.time(mf <- 
  mif(may, Nmif = 100, # Number of iterations
      start = theta.guess, # Starting value of parameters
      transform = TRUE, # work in transformed parameter space (e.g. log to avoid negative par values)
