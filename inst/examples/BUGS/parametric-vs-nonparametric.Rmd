...
jags.inits <- function(){
  list("logr0" 		= log(rlnorm(1, 0, .3)), 
       "logK" 		= log(10 * rlnorm(1, 1, .1)),
       "logtheta" = log(2 * rnorm(1, 1, .1)),  
       "stdQ"			= 0.2  * rlnorm(1, 0, .1),
       "stdR"			= 1e-5 * rlnorm(1, 0, .1),
       "x" 				= y,
       .RNG.name="base::Wichmann-Hill", .RNG.seed=123)
...
colnames(myers_pardist) = c("K", "r0", "theta", "stdQ", "stdR") # assumes they are in this order...
