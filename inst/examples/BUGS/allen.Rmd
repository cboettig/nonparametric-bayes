...
```{r}
gp <- bgp(X=obs$x, XX=x_grid, Z=obs$y, zcov=TRUE, verb=0,
...
K_prior <- function(x) dunif(x, 0.01, 20)
logr_prior <- function(x) dunif(x, -5, 5)
logtheta_prior <- function(x) dunif(x, -5, 5)
...
models <- data.frame(x=x_grid, GP=tgp_dat$y, True=true_means, MLE=est_means, Ricker.MLE=alt_means, Parametric.Bayes = par_bayes_means)
