...
```r
ggplot(dat) + geom_density(aes(value)) + xlim(0,1.1) + theme_bw() + xlab("value as fraction of the optimal")
```

![plot of chunk unnamed-chunk-14](http://farm4.staticflickr.com/3701/10965228284_be39534a45_o.png) 
...
```r
  ...
  sim_ids <-  levels(dat$simulation)


  require(reml)
  col.defs = c(model = "name of the model used to simulate the data",
               sapply(parameters, function(x) "a parameter of the model"),
  ...
  unit.defs = c(list(model = c("Myers" = "a 3 parameter model of Beverton-Holt-like recruitment with the addition of an Allee effect (and fourth parameter for scaling the log-normal growth noise); based on a Myers et al 1995 (doi:10.1126/science.269.5227.1106)", 
                ...
                lapply(parameters, function(x) "model parameter units, see model definitions"),
                list(replicate = rep_ids,
                     simulation = sim_ids, 
...
eml_write(dat, contact="Carl Boettiger <cboettig@gmail.com>")
```
