...
algorithms adjusted by approximations to keep computations feasible.
 ...
 \textbf{This part is overstated and unnecessary} Though machine
...
approximation to the state dynamics.
 ...
 Okay, perhaps that's novel, but it's pretty trivial. Isn't it obvious
to everyon that it's trivial?
...
unnecessary. Add comment / apology on single species dynamics
...
Figure 1 include curves drawn from the posterior density?
 ...
 Do we need more specifics on Gaussian process as an approximation to
parametric models? Discussion of Gaussian process vs other machine
...
Should we include any emprical examples? Using data simulated from a
specified model rather than empirical data permits the comparison
against the true underlying dynamics, setting a bar for the optimal
