...
of policy
\citetext{\citealp[\emph{e.g.}][]{Reed1979}; \citealp{Clark1986}; \citealp[@Roughgarden1997;][]{Sethi2005}},
parameteric uncertainty
\citetext{\citealp[\emph{e.g.}][]{Ludwig1982}; \citealp[@Hilborn1997;
@McAllister1998;][]{Schapaugh2013}}, and model or structural uncertainty
\citep[\emph{e.g.}][]{Williams2001, Cressie2009, Athanassoglou2012}. Of
these, structural uncertainty incorporates the least a priori knowledge
or assumptions and is generally the hardest to quantify. Typical
approaches assume a weak notion of uncertainty where a correct or
reasonable approximation of the dynamics must be identified from among a
handful of alternative models. Here we consider an approach that
...
attention\footnote{The concept of adaptive probing is one area that has
  ...
  Management'' approaches (e.g. \citet{Ludwig1982}) that can trade off
  short term utility by choosing actions that can reduce uncertainty.
  Adaptive probing strategies arise when it is valuable to intentionally
  force a system far from the observed values even when the expected
  value such actions is low, as it provides much faster learning and
  consequent reduction of model uncertainty that can allow greater value
  to be derived later on. For instance, \citet{Ludwig1982} show that it
  may be advantageous to fish an unexploited population very heavily at
  ...
  dangerous.} arises when applying a dynamical model outside the range
of data on which it has been estimated. This extrapolation uncertainty
is felt most keenly in decision-theoretic applications, as (a) exploring
the potential action space typically involves considering actions that
...
outcomes \citep[\emph{e.g.}][]{Weitzman2013}.

Concerns over the potential for tipping points in ecological dynamics
\citep{Scheffer2001} highlight the dangers of uncertainty in ecological
management and pose a substantial challenge to existing
decision-theoretic approaches \citep{Brozovic2011}. Because intervention
is often too late after a tipping point has been crossed (but see
\citet{Hughes2013}), management is most often concerned with avoiding
...
shift dynamics \citep[e.g.][]{Bestelmeyer2012}.

\emph{Map}

Here we illustrate how a stochastic dynamic programming (SDP) algorithm
{[}\citet{Mangel1988}; @Possingham1997, Marescot2013{]} can be driven by
the predictions from a Bayesian non-parametric (BNP) approach
\citep{Munch2005a}.\\This provides two distinct advantages compared with
contemporary approaches. First, using a BNP sidesteps the need for an
accurate model-based description of the system dynamics.\\Second, the
...
\begin{itemize}
\item
  Complex dynamics \citep{Glaser2013}(http://doi.org/10.1111/faf.12037
...
crucible for much of the founding work
\citep{Gordon1954, Reed1979, May1979, Ludwig1982} in managing ecosystems
under uncertainty. Global trends \citep{Worm2006} and controversy
\citep{Hilborn2007, Worm2009} have made understanding these challenges
...
harvested; see \citet{Halpern2013} for further discussion of this issue.
...
in the context of modeling dynamical systems \citep{Kocijan2005}, and
was first introduced in the context ecological modeling and fisheries
management in \citet{Munch2005}. An accessible and thorough introduction
to the formulation and use of GPs can be found in \citet{Rasmussen2006}.
...
details and code provided in the Appendix. \citet{Rasmussen2006}
provides an excellent general introduction to Gaussian Processes and
\citet{Munch2005} first discusses their application in the context of
...
Bayesian context using Gibbs sampling (implemented in R {[}@RTeam{]}
using jags, {[}@R2jags{]}). We choose uninformative uniform priors for
all parameters (See Appendix, Figures S1-S3, and Table S1, and the R
...
expected to be optimal \citep{Reed1979}.}
\end{figure}
...
expected to be optimal \citep{Reed1979}, whereby population levels below
...
\citep{Cuddington2013, Geritz2012}, such models can be not only
inaccurate but misleading when relied upon in a quantitative decision
making framework.
...
function appropriately (e.g.~see
\citep{Halpern2013}(http://doi.org/10.1073/pnas.1217689110 ``Achieving
the triple bottom line in the face of inherent trade-offs among social
equity, economic return, and conservation.'')) we will assume this is
given. (Nor is this approach necessarily constrained to maximizing the
...
damage that might be incurred; see \citet{Polasky2011}).
...
\citep{Williams2001, Athanassoglou2012}.
...
programming (SDP) \citep{Mangel1988}. Problems of this sort underpin
...
under uncertainty \citep{Mangel1988, Schapaugh2013}. Uncertainty in the
...
model choice or model averaging \citep{Williams2001, Athanassoglou2012}.
...
single species \citep[derived from fist principles by][]{Allen2005a} as
...
point \citep{Scheffer2001}.\\Using data simulated from a specified model
...
Once the posterior Gaussian process (GP) has been estimated
\citep[e.g.~see][]{Munch2005}, it is necessary to adapt it in place of
the parametric equation for the stochastic dynamic programming (SDP)
solution \citep[see][ for a detailed description of parametric SDP
methods]{Mangel1988} to the optimal policy. The essence of the idea is
straight forward -- we will use the estimated GP in place of the
...
previously proposed \citep{Munch2005} to a decision-theoretic framework.
...
\bibliography{nonparametric-bayes}
